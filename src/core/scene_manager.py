...
class SceneManager:
    ...
    def change_scene(self, scene_id: int) -> bool:
        """Change scene with dissolve crossfade if pattern is set"""
        try:
            with self._lock:
                ...
                if self.dissolve_patterns.current_pattern_id is not None:
                    from_led_array = self._get_current_led_array()
                    
                    self.current_scene_id = scene_id
                    self.current_scene = self.scenes[scene_id]
                    to_led_array = self._get_current_led_array()
                    ...
                    if pattern:
                        self.dissolve_transition.start_dissolve(
                            pattern, 
                            self.current_scene.led_count,
                            from_led_array,
                            to_led_array
                        )
                        self.dissolve_transition.from_scene_id = old_scene_id
                        self.dissolve_transition.to_scene_id = scene_id
    ...
    def change_effect(self, effect_id: int) -> bool:
        """Change effect with dissolve crossfade if pattern is set"""
        try:
            with self._lock:
                if not self.current_scene:
                ...
                if self.dissolve_patterns.current_pattern_id is not None:
                    ...
                    if pattern:
                        ...
                        self.dissolve_transition.from_effect_id = old_effect_id
                        self.dissolve_transition.to_effect_id = effect_id
    ...
    def change_palette(self, palette_id: int) -> bool:
        """Change palette with dissolve crossfade if pattern is set"""
        try:
            with self._lock:
                if not self.current_scene:
                ...
                if self.dissolve_patterns.current_pattern_id is not None:
                    ...
                    if pattern:
                        ...
                        self.dissolve_transition.from_palette_id = old_palette_id
                        self.dissolve_transition.to_palette_id = palette_id
